...
- `generation attempt`：一次具有唯一 `attempt_id` 的用户主动生成；重复提交同一ID不重复计数。
- `queued`：任务已通过额度校验，但尚未取得全站5个并发槽位。
- `running`：任务已取得槽位并进入实际生成。
- `queue full`：已有15个等待任务，新任务未进入模型调用。
- `rate limited`：按匿名身份或IP达到频率限制；dry-run期间只记录预计拦截。
- `LLM call`：一次真实发送到模型供应商的请求；同一attempt最多两次。
- `budget blocked`：每日调用、Token或金额达到硬阈值，新任务不再调用模型。
- `unauthorized access`：当前服务端匿名身份尝试读取不属于自己的结果或文件。

运行和安全日志只使用匿名身份哈希、IP哈希、`request_id`与`attempt_id`关联，不记录原始经历或简历正文。
