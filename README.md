...
每次生成写作、重试和复核合计最多两次调用。接收器、Binder 与最终交付前检查复用请求级验证结果，新项目正文退出模板前缀及 SOFT_DERIVATIONS 的竞争改写；其他安全检查和 legacy 保留。来源或复核失败不回填原句、不降级为包装成功。真实三次调用观察到原句回传、正反复核区分和改写改善，但最终模板的真实写作/复核/保存闭环因额度未完成，不能据此宣称普遍稳定。证据、测试、权限及回滚见 [阶段说明](docs/canonical-evidence-bounded-expression.md)。

v0.9.19.1.1 修正校园类型关系证据：产品名称中的校园活动、学生会等词与后续职责不再凭共现取得校园类型资格。参与对象须在局部语句中完整结束，组织任职使用独立关系；真实校园工作中的开发不反向覆盖已确认参与关系。保留显式类型优先级及既有科研规则，不改分区、事实、模型协议或技能展示。详见 [阶段说明](docs/experience-type-relation-evidence-correction.md)。

v0.9.19.1 对齐展示证据：Web 首经历预览与 DOCX 使用中性 bullet，不再添加“项目简介／经历简介、我的职责、技术细节”标签。保留正文、顺序、字段附件及持久化 revision；网页仍仅预览首经历和前三条详情。

当前重放发现“研究尚未形成论文，我也没有提出新的模型结构”在冻结前被错误授予资格。经批准，仅纠正既有 Claim 的主语与独立否定连接关系，约束继续沿原通道传给模型和 Gate；不新增隐藏位置或展示分类器，正常 Fact 仍完整唯一分配。“本地运行／仅供同学试用”保留。混合 Fact 内的名称、日期重复尚不能安全分离，明确保留；没有声称完整重放 v200—v204 历史请求。详见 [阶段说明](docs/canonical-presentation-evidence-alignment.md)。
...
真实发送的完整冻结事实、限制和非经历材料已覆盖本轮对照，未增加重复摘要、修改 Prompt 或模型协议。三份正常输入及保留样本通过控制返回保存/DOCX，不代表模型任意概括已获事实证明；模型自行扩大职责仍是已验证的残留，真实模型未验收。调用链、失败证据、测试及回滚见 [阶段说明](docs/canonical-summary-evidence-consumption-alignment.md)。
...
未分类的已验证工具进入现有其他类别；既有 Token 歧义检查和独立 legacy 行为保留。没有背景段也可直接生成经历，非经历证据不创建 owner，不改变经历事实、表头或模型协议。本版不修改 Prompt、个人优势或 Gate，未调用付费模型；历史请求目标及真实模型效果的证据限制见 [阶段说明](docs/non-experience-evidence-consumption-alignment.md)。

v0.9.18.2 对齐 Canonical 表头证据：已确认的原始边界记录经现有适配层传至资格判定，名称不再依赖推断 title 或把整个 owner 当作字段来源。课程/个人项目与校园活动的明确名称得到保留，企业和岗位按本地任职关系独立提取，部门修饰语不再混入岗位。

时间保留明确省略年份的完整范围、至今和学年；跨年歧义、冲突候选及未知字段仍待澄清，任务日期和毕业年份不得直接取代经历时间。本版不修改分区裁决、类型、Claim/Fact、模型协议或下游交付权限；仅经批准透传原始边界并迁移旧来源/待填写测试契约。验证、残留和回滚见 [阶段说明](docs/canonical-header-evidence-alignment.md)。

v0.9.18.1.1 修正未标注标题的自我介绍进入经历、求职与技能拼成实习关系，以及“目前尚未开展”取得肯定 Fact 资格的问题。首段也必须提供局部经历依据；背景、意向、通用能力仍保留原文范围供既有消费者使用，无法确认的片段保留为待澄清，不能默认创建项目。直接输入真实经历、同段背景与实际工作、显式岗位型标题继续支持。

本版仅修改既有分区、Semantic Role、Claim Resolution、Type Resolution 四个业务服务。否定按局部断言作用，独立已完成事实和必要限定保留；正式实习关系不再由目标岗位与远处技能动作拼接。未修改 Prompt、模型协议、表头或下游 Gate；名称、岗位、个人优势及 Gate 否定作用范围缺陷仍单独保留。详见 [阶段说明](docs/experience-admission-and-claim-qualification.md)。

v0.9.18.1 修正冻结前的 Claim 资格与类型关系证据，只修改现有 Claim Resolution 和 Type Resolution 两个业务服务。准确输入的当前重放中，“后续计划研究多语言标题处理”由 current/eligible 改为 planned/withheld，原文和限制保留，正式 Fact 从12条变为11条；“实验室设备登记系统”不再因产品名与职责动作邻近而取得科研资格，冻结为项目经历，10条 Fact、owner、名称资格及时间决定不变。

计划判断沿用局部活动关系，支持前置主语/后续语境及独立限制范围，不将“计划管理系统”“拟合模型”或已完成准备工作仅按词头排除。科研判定要求参与研究任务或明确组织内研究关系，普通开发不能靠词数压过成立的科研关系；真实显式类型优先级保留。类型服务原有的计划性前缀过滤尚不能全部退出，删除它会使“计划前端开发实习”旧反例回归，故保留并登记。模型协议、Ledger规则、表头Authority及交付链不变；未调用付费模型。详见 [阶段说明](docs/canonical-claim-qualification-and-type-evidence.md)。

v0.9.17.6 收束成功交付出口：Canonical 接收在 JSON 修复前要求供应商明确正常结束，截断、缺失或未知结束状态在既有次数内重试，耗尽后明确失败，不借解析错误降级成功。最终 Gate 不通过时不创建成功结果、Revision 成功日志或导出文件；保留输入和关联失败证据。warning/observe 仍按原标准处理，纯 JSON 降级内容不变，但同样必须经过最终 Gate。

Revision 完整性比较补入 intro 的 Fact/Claim 附件；不新增语义判断、不修改 Prompt、模型额度、Gate/Router 或旧结果。经批准，九项依赖“Gate 失败仍保存”的旧测试改为检查真实最终内容并拒绝保存，其余接收测试只补齐正常结束元数据。两份授权真实样本首轮正常结束并保存导出，Fact 分别保留 10/10、6/6；名称/分类和 INCOMPLETE_SENTENCE 警告仍存在，不能据两次成功宣称普遍稳定。详见 [交付闭环阶段说明](docs/canonical-delivery-closure-and-legacy-exit.md)。
...
专项检查实际正常/长输入/重试发送文本及完整任务快照，独立 legacy 模板内容在空白归一化后与基线一致。全量 1074 项离线测试通过，尚未进行真实模型验收；历史重复引用的具体位置及唯一根因仍缺少原始返回证明。科研误分类、个人优势差异和下游截断未纳入本补丁，详见 [Canonical Project Task Cleanup](docs/canonical-project-task-cleanup.md)。
...
本阶段只适配结构标题、同区块关系消费和原文来源区间；经批准，Ledger 不再按排版换行拆碎已选中的结构化 Claim，不改变事实选择或 eligibility。名称/时间/表头权威、Prompt 和下游包装不变。自由叙述保留原路径，不承诺任意排版或模型文案逐字一致。测试、限制与完整部署验收见 [Structured Experience Boundary Consistency](docs/structured-experience-boundary-consistency.md)。
...
| v0.9.14.1 | Structured Experience Boundary Consistency | 统一冒号/独立标题行；保留区块正文及原文来源；结构标题不建 Fact，同 owner 关系不因排版换行失效。 |
...
- [Scoped Professionalization and Packaging Enhancement](docs/scoped-professionalization-and-packaging.md) 说明软性职责扩写、三档包装强度及硬事实禁区。
