...
4. 超过三段主要经历：优先完整保留与目标岗位最相关的经历；其余内容只有在具备独立事实时才可按真实类型保留，不得生成“其他经历”或通用摘要项目。
...
- 项目标题只写规范名称，不得使用“我做过一个”“我独立完成了”“项目一”等口语句或序号前缀。
- 相同技术栈不代表相同经历；RAG 项目、RAG 评测平台和企业 RAG 实习必须依据各自 `source_experience_id`、目标、职责、指标和证据独立生成。
...
- 技能只能来自原文或对应 experience_id 的事实账本；目标岗位、知识补齐和面试准备不得反向证明技能。
- CodeBuddy、虚拟机属于开发工具与环境；LoRa、地磁传感器属于物联网与通信；地图 API、路线规划属于地图与路线服务；SSL 属于安全机制；Token 必须结合上下文判断，只有 JWT、鉴权、登录态、权限或令牌校验证据才能归为安全机制；回归分析、模型效果对比和数据可视化按对应能力分类。

## v0.6.7 技术术语消歧与输出相关性规则

- 不得根据孤立关键词推导技能类别，每项技能必须能对应到 experience_id / fact_id 的明确事实语境。
- “Token 消耗、Token 成本、输入输出 Token”表示大模型调用成本或上下文管理，不得归入安全机制；只有 JWT、Bearer、鉴权、认证、登录态、权限、刷新令牌等证据才可归入安全机制。
- “模型”需区分回归/分类等统计模型与大语言模型；“训练”需区分模型训练、课程训练和一般练习；“部署”需区分公网服务部署与本地演示；“用户”需区分真实用户证据与目标用户；“测试”需区分软件测试与 RAG 评测。
...
- 如果 Course-scoped Study MVP 是北辰 Agent / AI Study Assistant 的具体阶段，应只生成一个项目，可命名为“北辰 Agent / AI Study Assistant（Course-scoped Study MVP）”。
- 只有一段内容拥有独立目标、职责、事实链路和结果证据时，才能单独生成 project；不得把纯标题行生成成缺少事实的空壳项目。
- “项目名称｜身份｜时间”等标题行不得进入 project.details。
- 父产品与阶段项目合并时，保留全部不重复的技术、问题排查、数据隔离、日志、部署、指标和结果事实。
- 不得仅因两个项目都使用 RAG、React、FastAPI、SQLite 或面向相同岗位而合并；共享技术栈不等于同一项目实体。
- 每个独立项目仍需绑定自己的 source_experience_id；无法确认父子关系时保持独立并进入 missing_questions，不做低置信度强制合并。

## v0.6.9 正式经历有效性硬门槛

- 不得生成 name 或 meta 为“其他经历、其他项目、综合经历、综合经历项目、未命名经历”的 project。
- 每个 project 必须有具体名称，并至少包含一条属于本段 experience_id 的动作、功能、技术、职责、证据或结果事实。
