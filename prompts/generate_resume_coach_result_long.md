...
8. 经历边界隔离：Canonical 调用每个 project 必须包含内部字段 source_experience_id，独立 legacy 调用仍尽量包含；只能使用对应 experience_id 的事实，不得把 EXP-001 的技术、数据、成果写入 EXP-002。如果无法判断来源，把不确定内容放入 missing_questions / claims。
...
- 严禁生成 name 或 meta 为“其他经历、其他项目、综合经历、综合经历项目、未命名经历”的 project。
- 每个 project 必须具有具体名称和至少一条可绑定当前 experience_id / fact_id 的独立事实。
